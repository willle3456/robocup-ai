...
class FMT(PRM):
    ...
    def plan(self):
        #self.graph.clear()
        self.sample_space()
        #print 'FMT start: {0}'.format(self.start)
        ...
        #nn_z_dict = {z: nn_z}
        cost = {v: -1 for v in list(self.graph.nodes())}
        while z != self.goal:
            v_open_new = []
            nn_z = self.get_neighbors_in_range(z)
            x_near = [v for v in v_unvisited if v in nn_z]
            #print 'unvisited: {0}, x_near: {1}'.format(v_unvisited, x_near)
            for x in x_near:
                nn_x = self.get_neighbors_in_range(x)
                #nn_x.append(x)
                #nn_x_dict = {x: nn_x}
                print 'neighbors: {0}, open: {1}'.format(nn_x, v_open)
                y_near = [v for v in v_open if v in nn_x]
                print 'y_near: {0}'.format(y_near)
                y_min = self.lowest_cost(cost, y_near, x)
                if not collision_checker.is_collision(y_min, x, self.obstacles):
            ...
            print 'v_open_new: {0}'.format(v_open_new)   
    ...
    def get_neighbors_in_range(self, pt):
        ...
        neighbors = []
        list_nodes = [v for v in list(self.graph.nodes)]
        for node in list_nodes:
            if pt.get_distance(node) <= self.radius:
                #print 'hi'
                neighbors.append(node)
        return neighbors

    def get_cost(self, pt_1, pt_2):
        return pt_1.get_distance(pt_2)

    def lowest_cost(self, cost, pt_list, pt=None):
        node = pt_list[0]
        min_cost = cost[n] if cost[node] > 0 else 0

        if pt:
            min_cost += pt.get_distance(pt_list[0])
        
        for n in pt_list:
            tmp_cost = cost[n] if cost[node] > 0 else 0

            if pt:
                tmp_cost += pt.get_distance(n)
